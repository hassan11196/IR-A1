...
class Lexer(object):
    ...
    def word(self):
        ...
        while self.current_char is not None and (self.current_char in printable):
            result += self.current_char
            self.advance()
        return str(result)
...
#             term_row_right = term_doc_matrix_np[term_index_right]
            ...
            if left.inverse == True:
                left.value = '!' + str(left.value)
                term_row_left = self.index.get_term_postings([self.ps.stem(left.row)])
                left.inverse = False
            
            if right.inverse == True:
                right.value = '!' + str(right.value)
                term_row_right = self.index.get_term_postings([self.ps.stem(right.row)])
                right.inverse = False
